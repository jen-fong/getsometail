...
$('.dog').on('click', function(e) {
    ...
    $('#apiDiv').html('');
        ...
        console.log(locate);
        locate = $('#location').val();
        var queryURL = 'http://api.petfinder.com/pet.find?key=e87ed7f16feb54480c426720735345a9&animal=dog&breed=' + dogType + '&location=' + locate + '&format=json&callback=?';
    $.ajax({
        url: queryURL,
        method: 'GET',
        dataType: 'JSONP'
...
// Petfinder is annoying. We have to use this retarded way to pull the shelter info from the pet.find method and then use shelter.find to find 100 shelters in this location.
...
function getShelterId() {
    var shelterURL = 'http://api.petfinder.com/shelter.find?key=e87ed7f16feb54480c426720735345a9&count=100&location=' + locate + '&format=json&callback=?';
    $.ajax({
        url: shelterURL,
        method: 'GET',
        dataType: 'JSONP',
